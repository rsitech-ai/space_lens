...
| --- | --- | --- | --- |
| EXT-001 | New Apple identifier/profile and App Store Connect record not created | Account owner | After exact-action approval, create `com.rsitech.spacelens`, obtain a matching profile, and create version 1.0 build 1 under team `2NY8A789TN` |
...
| EXT-003 | Public Support URL prepared but not deployed | Support/account owner | Deploy `https://rsitech.ai/spacelens/support` and verify unauthenticated HTTP 200 |
| EXT-004 | Public Privacy URL prepared but not deployed | Privacy/account owner | Deploy `https://rsitech.ai/spacelens/privacy` and verify it matches actual operations |
| EXT-005 | RESOLVED: App Review contact supplied | Account owner | Enter Rafal Sikora, `info@rsitech.ai`, `private contact stored only in App Store Connect` when the app record exists |
...
| EXT-011 | RESOLVED: legal copyright/name confirmed | Legal owner | Owner confirmed `Rafal Sikora`; repository metadata was updated on 2026-07-15 |
| EXT-012 | Territory selection missing | Business/account owner | Free pricing and automatic release are confirmed; confirm all available territories or provide exclusions |
...
Owner also confirms that agreements, tax, banking, developer-program membership and the upload/submission account role are current. These attestations do not authorize an external mutation. App-ID creation, website deployment, App Store record creation, upload, TestFlight changes, submission, merge, tag and public release remain separate gated actions.
